...
void UDzX_RadialMenu::NativeTick(const FGeometry& MyGeometry, float DeltaTime)
{
	...
	if (bIsMenuActive && !bIsGamepadControlled)
	{
...
void UDzX_RadialMenu::PreselectButtonGamepad(float ThumbValue_X, float ThumbValue_Y)
{
	...
	if (GamePadPosition.Size() > 0.00f)
	{
		...
		if (bIsMenuActive)
		{
			...
			SegmentAngle= ((Angle.Yaw - 90) + (bIsClockwise ? -180 : 180)) * (bIsClockwise ? -1 : 1);

			//Locally get current segment ID from given angle.
			const auto _tempSegment = GetCurrentSegment(SegmentAngle);
			/*Compare if current segment is not equal to lastly updated segment(CurrentSegmentID) to avoid looping each frame if there is no change.
			Also check if button exist in Buttons container to avoid errors.
			*/
			if (_tempSegment != CurrentSegmentID && Buttons.Contains(_tempSegment))
...
bool UDzX_RadialMenu::IsInDeadzone()
{
	if (!PlayerController)
	{
		return true;
	...
	const auto& MouseLocal = GetRootWidget()->GetCachedGeometry().AbsoluteToLocal(MousePos);
	const auto& ViewportToLocal = GetRootWidget()->GetCachedGeometry().AbsoluteToLocal(RadialMenuCenter);
	const auto& MouseDistanceFromCenter = FVector2D(MouseLocal - ViewportToLocal).Size();
	const bool isInDeadZone = MouseDistanceFromCenter <= DivideMenu.DeadZoneRadius || (DivideMenu.OuterDeadZoneRadius > DivideMenu.DeadZoneRadius && MouseDistanceFromCenter >= DivideMenu.OuterDeadZoneRadius);
...
void UDzX_RadialMenu::DefaultBinding()
{
	...
	if (InputActionClick && enhancedInputComponent)
	{
		enhancedInputComponent->BindAction(InputActionClick, ETriggerEvent::Triggered, this, &UDzX_RadialMenu::ClickEvent);
	}
...
float UDzX_RadialMenu::GetAngleFromMouseCenter()
{
	if (!PlayerController)
	{
		PlayerController = Cast<APlayerController>(GetOwningPlayer());
	...
	*/
	...
	const auto& Alpha = FMath::Atan2(ReducedVector.X, ReducedVector.Y);
	...
	const auto& AimAtAngle = ((FMath::RadiansToDegrees(Alpha)) + (bIsClockwise ? -180 : 180)) * (bIsClockwise ? -1 : 1);
	SegmentAngle = AimAtAngle;
	//Return value
	return AimAtAngle;
}
...
int32 UDzX_RadialMenu::GetCurrentSegment(float Angle)
{
	...
	return DivideMenu.CurrentSegment(Angle);
}

int32 UDzX_RadialMenu::GetSelectedButton()
{
